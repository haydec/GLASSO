% Main function to fit a time-varying Graphical Lasso model with optional Lasso penalty, PSD constraint, and temporal smoothing
function Theta_optimal = fitTimeVaryingGraphicalLassoSmoothing(Theta0, S, lasso_penalty, beta, num_samples,enforce_psd)
    % Fits a time-varying graphical lasso model with optional sparsity, PSD constraint, and temporal smoothing.
    %
    % Input:
    ...
    % - beta: Temporal smoothing penalty parameter
    ...
    % Define the objective function as the sum of individual objectives plus temporal smoothing penalty
    objective = @(Theta_vec) sumGraphicalLassoObjective(Theta_vec, S, lasso_penalty, beta, n, T, num_samples);
...
% Sum of graphical lasso objectives across all time points with temporal smoothing
function f = sumGraphicalLassoObjective(Theta_vec, S, lasso_penalty, beta, n, T, num_samples)
    % Compute the sum of graphical lasso objectives across time points with temporal smoothing penalty
    % Input:
    ...
    % Iterate over each time point to calculate the graphical lasso objective
    Theta_matrices = zeros(n, n, T);
    for t = 1:T
        ...
        % Store the reconstructed Theta matrix for temporal smoothing
        Theta_matrices(:, :, t) = Theta;
        idx = idx + d;
    end

    % Add temporal smoothing penalty between consecutive precision matrices
    for t = 2:T
        diff_matrix = Theta_matrices(:, :, t) - Theta_matrices(:, :, t-1);
        temporal_penalty = beta * norm(diff_matrix, 'fro')^2; % Normalized penalty
        f = f + temporal_penalty;
    end
